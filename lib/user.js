...
module.exports = {
  ...
  getBannedUsers (offset, limit, callback) {
    winston.info("DB.User: Get banned users with offset:", offset, "and limit:", limit);
    if (!offset && !limit) {
      return callback(null, []);
    }

    User.find().where("ban").gt(0).sort('email').skip(offset).limit(limit).select("email ban report ip authToken").lean().exec(callback);
  },
