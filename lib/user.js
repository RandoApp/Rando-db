...
module.exports = {
  ...
  getAllLightInAndOutRandosByEmailForExchange(email, callback) {
    winston.info("DB.User: getAllLightInAndOutRandosByEmailForExchange by email:", email);
    User.findOne({ email }, {
      email: 1,
      authToken: 1,
      ip: 1,
  ...
  getLightOutRandosForPeriod(start, end, callback) {
  ...
  updateDeleteFlagForInRando(email, randoId, deleteFlag, callback) {
    ...
    User.update({ $and: [{ email }, { "in.randoId": randoId }] }, {
      $set: {
  ...
  updateReportFlagForInRando (email, randoId, reportFlag, callback) {
    winston.info("DB.User: updateReportFlagForInRando by email:", email, "and randoId:", randoId, "reportFlag:", reportFlag);
    User.update({ $and: [{ email }, { "in.randoId": randoId }] }, {
      $set: {
        "in.$.report": reportFlag
      }
    }, (err, data) => {
      if (callback) {
        return callback(err, data);
  ...
  updateRatingForOutRando(email, randoId, rating, callback) {
    winston.info("DB.User: updateRatingForOutRando by email:", email, "and randoId:", randoId, "rating:", rating);
    User.update({ $and: [{ email }, { "out.randoId": randoId }] }, {
      $set: {
        "out.$.rating": rating
      }
  ...
  updateRatingForInRando(email, randoId, rating, callback) {
    winston.info("DB.User: updateRatingForInRando by email:", email, "and randoId:", randoId, "rating:", rating);
    User.update({ $and: [{ email }, { "in.randoId": randoId }] }, {
      $set: {
        "in.$.rating": rating
      }
